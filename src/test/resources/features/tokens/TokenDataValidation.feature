...
Scenario: Validate token data field constraints
  ...
  * def tokenData = response.data
  ...
  And match tokenData.supply == '#? _ <= 1000000000'
  ...
  And match tokenData.image == '#? _ == null || _ == "" || _ matches "^https://static\\.cloud-service-app\\.com.*"'
  
  # Validate metadata_uri if present
  And match tokenData.metadata_uri == '#? _ == null || _ == "" || _ matches "^https://ipfs\\.io/ipfs/.*" || !(_ contains "/ipfs/")'
  
  # Validate authorities
  And match tokenData.mint_authority == '#? _ == null || _ matches "^[1-9A-HJ-NP-Za-km-z]{32,44}$"'
  And match tokenData.freeze_authority == '#? _ == null || _ matches "^[1-9A-HJ-NP-Za-km-z]{32,44}$"'
  
  # Validate bonding curves
  And match tokenData.bonding_curve == '#? _ == null || _ matches "^[1-9A-HJ-NP-Za-km-z]{32,44}$"'
  And match tokenData.associated_bonding_curve == '#? _ == null || _ matches "^[1-9A-HJ-NP-Za-km-z]{32,44}$"'
...
Scenario: Validate token metadata field constraints
  ...
  * def tokenData = response.data
  ...
  # Validate metadata image if present
  And match tokenData.metadata.image == '#? _ == null || _ == "" || _ matches "^https://ipfs\\.io/ipfs/.*" || !(_ contains "/ipfs/")'
  
  # Validate optional metadata fields don't exist if empty
  And match tokenData.metadata.description == '#? _ == null || _ != ""'
  And match tokenData.metadata.twitter == '#? _ == null || _ != ""'
  And match tokenData.metadata.website == '#? _ == null || _ != ""'
  And match tokenData.metadata.telegram == '#? _ == null || _ != ""'
  And match tokenData.metadata.uri == '#? _ == null || _ != ""' 