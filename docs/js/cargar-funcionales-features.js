async function cargarYMostrarFeaturesFuncionales() {
    const indexResp = await fetch('karate-reports/functional/index.json');
    const featuresList = await indexResp.json();

    // Definición de categorías y palabras clave asociadas
    const categorias = [
        { nombre: 'Smoke Tests', icono: '🔥', keywords: ['Smoke'] },
        { nombre: 'Regression Tests', icono: '🔁', keywords: ['Regression'] },
        { nombre: 'Integration Tests', icono: '🔗', keywords: ['Integration'] },
        { nombre: 'End2End Tests', icono: '🛤️', keywords: ['End2End', 'E2E'] },
        { nombre: 'Negative Tests', icono: '❌', keywords: ['Negative'] },
        { nombre: 'Positive Tests', icono: '✅', keywords: ['Positive'] },
        { nombre: 'Complete Suite', icono: '📋', keywords: ['Complete'] },
    ...
    for (const cat of categorias) {
        ...
        for (const featureName of featuresCat) {
            try {
                const featureResp = await fetch(`karate-reports/functional/${featureName}.json`);
...
window.onload = cargarYMostrarFeaturesFuncionales; 